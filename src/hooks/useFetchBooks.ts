	...
	const fetchBooks = useCallback(async () => {
		...
		try {
			const response = await fetch(
				`https://www.googleapis.com/books/v1/volumes?q=subject:${query}&orderBy=relevance&langRestrict=en&fields=items(id,volumeInfo(title,authors,imageLinks/thumbnail,publishedDate),searchInfo)`
			);
